...
class BaseBatchAnalyzer(ABC):
    ...
    def __init__(self, base_log_dir: str, target_ids: Optional[Set[str]] = None, overwrite: bool = False):
        ...
        self._print_lock = threading.RLock()  # RLock: reentrant, same thread can acquire multiple times
    ...
    def run_single_analysis(self, identifier_info: Dict[str, Any], source_file: Path,
        ...
        self._ts_print(f"Command: {' '.join(cmd)}")
        self._ts_print(f"🚀 Executing analysis for: {identifier}")
        self._ts_print(f"   📁 Log directory: {self.base_log_dir / subdir_name}")
        ...
        try:
            ...
            if result.returncode == 0:
                self._ts_print(f"   ✅ Process executed successfully: {identifier}")
                subprocess_success = True
            else:
                self._ts_print(f"   ❌ Process execution failed: {identifier} (return code: {result.returncode})")
                subprocess_success = False

        except subprocess.TimeoutExpired:
            self._ts_print(f"   ⏰ Execution timeout: {identifier}")
            subprocess_success = False
        except Exception as e:
            self._ts_print(f"   ❌ Execution exception: {identifier} - {e}")
            subprocess_success = False
        ...
        if subprocess_success:
            if self.is_analysis_successful(subdir_name):
                self._ts_print(f"   ✅ Analysis result quality check passed: {identifier}")
                return True
            else:
                self._ts_print(f"   ⚠️ Process succeeded but result quality is below standard: {identifier}")
                return False
        else:
            return False
    ...
    def run_batch_analysis(self, source_file: Path, max_retries: int = 2, **kwargs) -> None:
        ...
        skip_confirm = kwargs.get("skip_confirm", False)
        if not skip_confirm:
        ...
        if parallel > 1:
            print(f"\n🚀 Step 3: Starting Agent analysis (parallel mode, {parallel} workers)...")
            self._run_analysis_parallel(identifiers, source_path, max_retries, parallel, **kwargs)
        else:
            print(f"\n🚀 Step 3: Starting Agent analysis...")
            self._run_analysis_with_retry(identifiers, source_path, max_retries, **kwargs)


    def _run_analysis_parallel(self, identifiers: List[Dict[str, Any]], source_path: Path,
                               max_retries: int, max_workers: int, **kwargs) -> None:
        """Run analysis in parallel using a thread pool.

        Each company gets its own thread. Within a thread, retries are serial
        (run → check → retry if failed), but different companies run concurrently.
        """
        self._completed_count = 0
        self._total_count = len(identifiers)
        failed_lock = threading.Lock()
        all_failed: List[Dict[str, Any]] = []

        def _analyze_one(info: Dict[str, Any]) -> bool:
            """Run one company with independent retry. Returns True on success."""
            identifier = info.get("identifier", "unknown")
            subdir = self.get_subdir_name(identifier)

            for attempt in range(max_retries + 1):
                if attempt > 0 or self.overwrite:
                    self.remove_existing_log_dir(subdir)

                ok = self.run_single_analysis(info, source_path, subdir, **kwargs)

                if ok and self.is_analysis_successful(subdir):
                    tag = f"(retry #{attempt})" if attempt > 0 else ""
                    with self._print_lock:
                        self._completed_count += 1
                        print(f"[{self._completed_count}/{self._total_count}] ✅ {identifier} {tag}")
                    return True
                else:
                    if attempt < max_retries:
                        self._ts_print(f"   ⚠️ {identifier} attempt {attempt+1} failed, retrying...")
            ...
            with self._print_lock:
                self._completed_count += 1
                print(f"[{self._completed_count}/{self._total_count}] ❌ {identifier} (all {max_retries+1} attempts failed)")
            return False

        self._ts_print(f"Parallel mode: {max_workers} workers, {self._total_count} companies")
        self._ts_print("=" * 50)

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(_analyze_one, info) for info in identifiers]
            for future in as_completed(futures):
                try:
                    future.result()
                except Exception as e:
                    self._ts_print(f"   ❌ Worker thread crashed: {e}")

        # --- Summary (same logic as _run_analysis_with_retry) ---
        actual_ids = self._get_actual_identifiers_from_logs()
        actual_failed_ids = self.get_failed_identifiers([])
        actual_success = len(actual_ids) - len(actual_failed_ids)

        self._ts_print("\n" + "=" * 50)
        self._ts_print("📊 Batch analysis completed!")
        self._ts_print(f"✅ Success: {actual_success}")
        self._ts_print(f"❌ Failed: {len(actual_failed_ids)}")
        if len(actual_ids) > 0:
            self._ts_print(f"📈 Success rate: {actual_success / len(actual_ids) * 100:.1f}%")
        self._ts_print(f"📁 Logs saved to: {self.base_log_dir}")

        if actual_failed_ids:
            self._ts_print(f"\n❌ The following identifiers failed analysis:")
            for fi in actual_failed_ids:
                self._ts_print(f"   - {fi.get('identifier', 'unknown')}")
    ...
    def retry_failed_analyses(self, max_retries: int = 2, **kwargs) -> None:
        ...
        parallel = kwargs.pop("parallel", 1)
        if parallel > 1:
            self._run_analysis_parallel(failed_identifiers, Path("."), max_retries, parallel, **kwargs)
        else:
