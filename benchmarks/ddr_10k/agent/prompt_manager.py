...
class PromptManager:
    ...
    def __init__(self, auto_finish: bool = True, semantic_manifest: str = ""):
...
Carefully use "FINISH:" in your message since it will immediately end the session. Think twice before using it."""
    ...
    def build_system_prompt_with_task(self, task: str) -> str:
        ...
        if self.semantic_manifest:
            enhanced_prompt += (
                "\n\n<ontology_session_manifest>\n"
                f"{self.semantic_manifest}\n"
                "</ontology_session_manifest>"
            )
...
3. The insight from data should answer the question raised in the reason to execute this tool. Focus on this point.
4. Keep all the data or statistics needed in your generated insight.
