...
class SQLiteMCPServer(BaseMCPServer):
    ...
    # C12: Column name hallucination repair map (deterministic, auditable)
    # Agent hallucinates these → database uses these
    _COLUMN_REPAIR_MAP = {
        # financial_facts table
        "concept_name": "fact_name",
        "concept": "fact_name",
        "value": "fact_value",
    ...
    def __init__(self, db_path: str, enable_detailed_logging: bool = True):
        ...
        # For extra CSV columns (Task 13.2)
        self._last_parsed_fact_names = ""
        self._last_fiscal_years = ""

        # C12: SQL repair tracking
        self._sql_repair_count = 0
        self._sql_repair_log = []
    ...
    def _execute_query(self, query: str, limit: int = None) -> Dict[str, Any]:
        ...
        try:
            with self._get_db_connection() as conn:
                ...
                # Extract parsed_fact_names and fiscal_years for CSV logging (Task 13.2)
                self._last_parsed_fact_names = self._extract_fact_names_from_query(original_query, columns)
                self._last_fiscal_years = self._extract_fiscal_years_from_result(columns, rows)
            ...
            # C12: Try to auto-repair schema hallucination errors
            if "no such column:" in error_msg.lower():
                repaired_query = self._try_repair_sql(original_query, error_msg)
                if repaired_query and repaired_query != original_query:
                    try:
                        with self._get_db_connection() as conn:
                            # Re-apply limit if needed
                            exec_query = repaired_query
                            if limit_applied:
                                if 'LIMIT' not in repaired_query.upper():
                                    exec_query = f"{repaired_query.rstrip(';')} LIMIT {limit}"
                            cursor = conn.execute(exec_query)
                            ...
                            result = {"cols": columns, "data": rows, "count": len(rows)}
                            result["sql_repaired"] = True
                            result["original_sql"] = original_query
                            result["repaired_sql"] = repaired_query

                            self._sql_repair_count += 1
                            self._sql_repair_log.append({
                                "original": original_query[:200],
                                "repaired": repaired_query[:200],
                                "error": error_msg[:100],
                            })
                            self.logger.info(f"🔧 SQL auto-repaired (#{self._sql_repair_count}): "
                                             f"{error_msg[:80]} → success")
    ...
    def _extract_fact_names_from_query(self, query: str, result_columns: List[str]) -> str:
        """Extract XBRL fact_names from SQL query (WHERE fact_name = 'X' / IN patterns).

        Returns comma-separated fact_name string, or empty string.
        """
        fact_names = set()
        xbrl_pattern = re.compile(r'^[A-Z][a-zA-Z0-9]+$')

        # Extract fact_name = 'X' (individual values)
        for m in re.finditer(r"fact_name\s*=\s*'([^']+)'", query, re.IGNORECASE):
            fn = m.group(1)
            if xbrl_pattern.match(fn) and len(fn) > 3:
                fact_names.add(fn)

        # Extract fact_name IN ('X', 'Y', ...)
        for m in re.finditer(r"fact_name\s+IN\s*\(([^)]+)\)", query, re.IGNORECASE):
            inner = m.group(1)
            for token in inner.split(','):
                fn = token.strip().strip("'\" ")
                if xbrl_pattern.match(fn) and len(fn) > 3:
                    fact_names.add(fn)

        # Fallback: check result columns for PascalCase identifiers
        if not fact_names:
    ...
    def _extract_fiscal_years_from_result(self, columns: List[str], rows: List[List]) -> str:
        """Extract fiscal years from SQL result data.

        Looks for fiscal_year / fy / year columns and returns unique sorted years.
        """
        year_col_indices = []
        for i, col in enumerate(columns):
            col_lower = col.lower()
            if col_lower in ('fiscal_year', 'fy', 'year', 'fiscalyear', 'fy_year'):
                year_col_indices.append(i)

        years = set()
        for row_data in rows:
            for idx in year_col_indices:
                if idx < len(row_data) and row_data[idx] is not None:
                    try:
                        year_val = int(row_data[idx])
    ...
    def _get_extra_csv_headers(self) -> List[str]:
        """Override: add parsed_fact_names and fiscal_years columns."""
        return ["parsed_fact_names", "fiscal_years"]

    def _get_extra_csv_values(self) -> Dict[str, Any]:
        """Override: return stored parsed_fact_names and fiscal_years values."""
        values = {
            "parsed_fact_names": self._last_parsed_fact_names,
            "fiscal_years": self._last_fiscal_years,
        }
        # Reset after logging
        self._last_parsed_fact_names = ""
        self._last_fiscal_years = ""
    ...
    def _try_repair_sql(self, query: str, error_msg: str) -> Optional[str]:
        """C12: Attempt to repair schema hallucination errors in SQL.

        When SQLite reports 'no such column: X', check if X is a known
        hallucination pattern with a deterministic fix. Only applies
        unambiguous, conservative repairs.
        """
        ...
        # Handle table-qualified names (e.g., f.form_type → f.form)
        if '.' in missing_col:
            parts = missing_col.rsplit('.', 1)
            alias_prefix = parts[0]
            base_col = parts[1]
            if base_col in self._COLUMN_REPAIR_MAP:
                correct_col = self._COLUMN_REPAIR_MAP[base_col]
                old_ref = re.escape(missing_col)
                new_ref = f"{alias_prefix}.{correct_col}"
                return re.sub(r'(?<![a-zA-Z0-9_])' + old_ref + r'(?![a-zA-Z0-9_])', new_ref, query)
            return None

        # Handle unqualified column names
        if missing_col in self._COLUMN_REPAIR_MAP:
            correct_col = self._COLUMN_REPAIR_MAP[missing_col]
            # Use word-boundary replacement to avoid partial matches
            return re.sub(
                r'(?<![a-zA-Z0-9_])' + re.escape(missing_col) + r'(?![a-zA-Z0-9_])',
                correct_col, query,
            )
        return None
